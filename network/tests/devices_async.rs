use network::{
    ...
    r#async::{
        ClientAsync, NetworkDeviceAsync, TCPClientAsync, TCPServerAsync, UDPClientAsync,
        UDPServerAsync,
    },
    Result,
};
...
async fn sockets() -> Result<()> {
    ...
    let socket2_udp: NetworkDeviceAsync<UDPServerAsync> =
        NetworkDeviceAsync::new(socket2, "127.0.0.1:8001").await?;
