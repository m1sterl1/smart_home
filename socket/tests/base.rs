...
fn main(){
    ...
    assert_eq!(response, CommandResponse::GetState(Err("No socket".to_string())));

    // Turn off socket and get state
    client.turn_off("First").unwrap();
    let response = client.get_state("First").unwrap();
    assert_eq!(response, CommandResponse::GetState(Ok("State: off, power consumption 0.0W".to_string())));
}
